...
lights = [EnvironmentLight(1.0, img'[end:-1:1, :]), 
    PointLight(Vec3f(1,0.25,1.0), RGBf(85.0, 85.0, 85.0))]
...
screen=RPRMakie.RPRScreen(size(ax.scene); plugin=RPR.Northstar, iterations=10_000)
matsys=screen.matsys
meshscatter!(ax, toPoints3D; markersize = ms / 20 .+ 0.001, color = mag,
    colormap =Reverse(:linear_bmy_10_95_c78_n256))
#box
...
GLMakie.activate!()
zoom!(ax.scene, cameracontrols(ax.scene), 0.85)
...
save("./imgs/earthquakes_clean.png", imageOut) # save just screen scene.
saveRPR("./imgs/earthquakes", imageOut,  resolution=(1200, 1200))